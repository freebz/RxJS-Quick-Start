...
export default class Map {
    ...
    // 지도 위체 표시되는 정보창(infowindow)을 닫는다.
    closeInfoWindow() {
	this.infowindow.close();
    }
    ...
    createDragend$() {
	return formEvent(this.naverMap, "dragend") // 지도 영역을 dragend 했을 때
	.pipe(
	    map(({ coord }) => ({
		latitude: coord.y,
		longitude: coord.x
	...
	.pipe(
		...
		id: overlay.getOptions("id"), // 버스정료수ID 정보를 얻음
	...
	.pipe(
		...
		marker.setOptions("name", stationsstationName);
		return marker;
	    })),
	    scan((prev, markers) => {
		// 이전 markers 삭제
	...
	const list = buses.map(bus => (`<dd>
		<a href="#${bus.routeId}_${bus.routeName}">
		    <strong>${bus.routeName}</strong> <span>${bus.regionName}</span> <span class="type ${getBuesType(bus.routeTypeName)}">${busrouteTypeName}</span>
		</a>
	    </dd>`)).join("");
